...
INTERRUPT_HANDLER(RTC_CSSLSE_IRQHandler,4)//RTC / CSS_LSE Interrupt routine.
 {//wakeup
   RTC->ISR2 = 0;
   wakeup();
 }
...
{ /* 
  uint8_t i, temp;
     //тут нужно проверить, какой из 4-х портов дал прерывание
  static uint8_t portB = 0;
  EXTI->SR2 = 1;
  temp = GPIOB->IDR;
  for(i=0; i<4; i++)
  {
  	if( (portB & (1<<i)) != (temp & (1<<i)) )
  	{
		isrImpl(i);
  	}
  }
...
INTERRUPT_HANDLER(TIM4_UPD_OVF_TRG_IRQHandler,25)//TIM4 Update/Overflow/Trigger Interrupt routine.
{  
